...
def get_best_model(series_data, target_column, is_for_eval=False):
    """Membangun model terbaik berdasarkan target. Jika untuk evaluasi, gunakan data latih."""
...
def get_evaluation_metrics(data, target_column):
    """
    [FUNGSI BARU] Menghitung metrik evaluasi dengan metode train-test split
    untuk mencocokkan hasil dari notebook.
    """
    # Menentukan titik pisah data latih dan uji (misal, 12 bulan terakhir sebagai data uji)
...
if data_bulanan is not None:
    ...
    if predict_button:
        with st.spinner(f"Membuat prediksi dan evaluasi untuk {target_display_name}..."):
            # 1. Hitung metrik evaluasi secara dinamis dengan benar

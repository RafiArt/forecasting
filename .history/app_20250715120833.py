...
def load_custom_css():
    ...
    <style>
        ...
        /* Tombol Utama - warna dasar hitam, hover oranye */
        .stButton button {
        ...
        /* Memberi sudut membulat pada container chart */
        .stPlotlyChart {
            border-radius: 20px;
            overflow: hidden;
        }
...
def train_and_predict(data, target_column, n_periods):
    ...
    # [PERUBAHAN UTAMA] Logika untuk memilih model terbaik secara otomatis
    if target_column == 'Rombong':
        # Model terbaik untuk Rombong adalah SARIMA
...
with st.sidebar:
    ...
    # [PERUBAHAN] Mengubah label "Penjualan Rombong" menjadi "Bantuan Rombong"
    ...
    # [PERUBAHAN] Menghapus pilihan model, diganti dengan informasi model terbaik
    st.markdown("---")
    st.write("**Model Terbaik yang Digunakan:**")
    if target_column_name == "Rombong":
        st.info("SARIMA (1,0,1)(1,1,1,12)")
    else:
...
if data_bulanan is not None:
    ...
    if predict_button:
        with st.spinner(f"Membuat prediksi {target_display_name} dengan model terbaik..."):
            # [PERUBAHAN] Tidak perlu lagi mengirim pilihan model
            prediction_df, model_fit, model_name = train_and_predict(data_bulanan, target_column_name, periods_input)
        
        st.markdown("---")
        # [PERUBAHAN] Judul hasil menggunakan nama model yang dikembalikan dari fungsi

...
def load_custom_css():
    ...
    <style>
        ...
        /* [PERBAIKAN] Warna teks utama menjadi hitam */
...
def load_data(uploaded_file):
    ...
    try:
        ...
        # [PERBAIKAN UTAMA] Set index dan langsung urutkan untuk mencegah error
...
# HALAMAN UTAMA
if data_bulanan is not None:
    st.markdown(f'<h1>📈 Dashboard Forecasting: {target_display_name}</h1>')
    st.markdown("---")
    ...
    if filtered_data.empty:
        ...
        st.markdown(f'<h2>Grafik Data Aktual (Rentang: {start_date.strftime("%b %Y")} - {end_date.strftime("%b %Y")})</h2>')
        ...
        if 'predict_button' in locals() and predict_button:
            ...
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.markdown(f'<h2>Hasil Prediksi Menggunakan {model_name}</h2>')
            fig_pred = go.Figure()
            ...
            with col1:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.markdown('<h4>🔢 Tabel Nilai Prediksi</h4>')
            ...
            with col2:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.markdown('<h4>📝 Evaluasi Model Terbaik</h4>')
    ...
    st.markdown("""
        <div class="card" style="text-align: center;">
            <h2 style="color: black;">✨ Selamat Datang di Dashboard Forecasting BAZNAS</h2>
            <p style="color: black; font-size: 1.1em;">Aplikasi ini dirancang untuk membantu Anda menganalisis dan memprediksi data bantuan mustahik dengan mudah dan akurat.</p>
            <hr>
            <div style="display: flex; justify-content: space-around; text-align: left;">
                <div style="flex: 1;">
                    <h4>Fitur Utama:</h4>
                    <ul style="list-style-position: inside;">
                        <li>Analisis Data Historis</li>
                        <li>Prediksi Akurat</li>
                        <li>Visualisasi Interaktif</li>
                    </ul>
                </div>
                <div style="flex: 1; margin-top: 20px;">
                    <h4 style="color: #00502D;">⬅️ Mulai Sekarang!</h4>
                    <p>Unggah file Excel Anda pada sidebar untuk memulai analisis.</p>
                </div>
            </div>
        </div>

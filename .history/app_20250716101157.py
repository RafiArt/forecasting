...
def load_data(uploaded_file):
    ...
    try:
        ...
        st.error(f"Gagal memuat data dari '{source}': {e}. Pastikan format file dan nama kolom sudah benar.")
        return None
...
with st.sidebar:
    ...
    # Memuat data untuk mendapatkan rentang tanggal
    data_bulanan = load_data(uploaded_file)
    
    # [FITUR BARU] Date Picker untuk filter data
    if data_bulanan is not None:
        min_date = data_bulanan.index.min().date()
        max_date = data_bulanan.index.max().date()

        st.write("**Filter Rentang Waktu (Visualisasi):**")
...
if data_bulanan is not None:
    # Filter data hanya untuk visualisasi grafik pertama
    filtered_data = data_bulanan.loc[start_date:end_date]
    if filtered_data.empty:
        st.warning("Rentang data yang Anda pilih tidak valid. Menampilkan seluruh data.")
        filtered_data = data_bulanan
        
    st.markdown(f'<h3 style="color: black;">Grafik Data Aktual: {target_display_name}</h3>', unsafe_allow_html=True)
    fig_actual = go.Figure()
    # Gunakan filtered_data untuk plot
    fig_actual.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data[target_column_name], mode='lines+markers', name='Data Aktual', line=dict(color='#00502D', width=3)))
    ...
    if predict_button:
        with st.spinner(f"Membuat prediksi dan evaluasi untuk {target_display_name}..."):
            # Evaluasi dan training tetap menggunakan data penuh (data_bulanan)
        ...
        fig_pred = go.Figure()
        # Grafik prediksi menampilkan seluruh sejarah untuk konteks

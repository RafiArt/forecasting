...
if data_bulanan is not None:
    ...
    if predict_button and data_bulanan is not None:
        ...
        with col2:
            with st.container():
                st.markdown('<div class="custom-container">', unsafe_allow_html=True)
                st.markdown(f'<h3 style="color: black; text-align: center;">Hasil Prediksi Menggunakan {model_name}</h3>', unsafe_allow_html=True)
                fig_pred = go.Figure()
        ...
        # Menjaga layout kolom kedua tetap ada meskipun kosong
        with col2:
            st.empty()

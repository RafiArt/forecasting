...
def load_custom_css():
    ...
    <style>
        ...
        /* Teks pada metrik evaluasi */
        [data-testid="stMetric"] {
            background-color: rgba(255, 255, 255, 0.5); border-radius: 8px; padding: 10px;
        }
...
def get_evaluation_metrics(data, target_column):
    """
    [PERBAIKAN UTAMA] Menghitung metrik dengan train-test split yang sesuai dengan masing-masing notebook.
    """
    ...
    fit_model = model_for_eval.fit(disp=False)
...
def train_and_predict_future(data, target_column, n_periods):
    """Melatih model pada SEMUA data untuk membuat prediksi masa depan yang paling akurat."""
    model_for_future, model_name = get_best_model(data[target_column], target_column)
    fit_model = model_for_future.fit(disp=False)

...
def load_data(uploaded_file):
    """
    Memuat dan memproses data. Prioritaskan file yang di-upload.
    Jika tidak ada, coba muat file lokal 'data.xlsx'.
    """
    try:
        if uploaded_file is not None:
            # Jika ada file yang di-upload, gunakan file itu
            data = pd.read_excel(uploaded_file)
        else:
            # Jika tidak, gunakan file lokal sebagai default
        ...
        st.error(f"Gagal memuat data: {e}. Pastikan format file Excel dan nama kolom sudah benar (Bulan Tahun, Modal Usaha, Rombong).")
        return None
...
with st.sidebar:
    ...
    # [PERUBAHAN UTAMA] Menambahkan fitur upload file
    uploaded_file = st.file_uploader(
        "Upload Dataset Baru (Opsional)",
        type=['xlsx', 'xls']
    )
    st.markdown("---")
...
# Memuat data setelah widget di sidebar di-render
...
if data_bulanan is not None:
    # Selalu tampilkan grafik data aktual
...
elif data_bulanan is None and uploaded_file is None:
    st.info("Silakan upload file Excel untuk memulai analisis, atau letakkan file 'data.xlsx' di folder yang sama.")